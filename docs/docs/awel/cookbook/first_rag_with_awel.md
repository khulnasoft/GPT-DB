...
To store the knowledge in a vector store, we need an embedding model, GPT-DB supports 
...
chunks = asyncio.run(assembler_task.call("https://docs.gptdb.site/docs/latest/awel/"))
...
To build a RAG program, we need a LLM, here are some of the LLMs that GPT-DB supports:
...
from gptdb.core.awel import InputOperator, JoinOperator, InputSource
