...
GPTDB_API_KEY=gptdb
APP_ID={YOUR_APP_ID}
curl -X GET "http://localhost:5670/api/v2/serve/apps/$APP_ID" -H "Authorization: Bearer $GPTDB_API_KEY"
```
 </TabItem>
...
```python
from gptdb.client import Client
from gptdb.client.app import get_app
...
curl -X GET 'http://localhost:5670/api/v2/serve/apps' -H "Authorization: Bearer $GPTDB_API_KEY"
```
 </TabItem>
