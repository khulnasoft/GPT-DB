...
// 对话气泡
const ChatDialog: React.FC<{ message: IChatDialogueMessageSchema; index: number }> = ({ message, index }) => {
  ...
  const { value } = useMemo<{ relations: string[]; value: string; cachePluginContext: GPTDBView[] }>(() => {
    ...
    const cachePluginContext: GPTDBView[] = [];

    let cacheIndex = 0;
    const result = value.replace(/<gptdb-view[^>]*>[^<]*<\/gptdb-view>/gi, (matchVal) => {
      try {
        const pluginVal = matchVal.replaceAll('\n', '\\n').replace(/<[^>]*>|<\/[^>]*>/gm, '');
        const pluginContext = JSON.parse(pluginVal) as GPTDBView;
    ...
    >
      ...
      {isRobot && (
        <div className="flex max-w-full flex-col flex-wrap bg-white dark:bg-[rgba(255,255,255,0.16)] p-3 rounded-xl rounded-bl-none">
          {typeof context === 'string' && scene === 'chat_agent' && <MarkdownContext>{formatMarkdownValForAgent(value)}</MarkdownContext>}
          {typeof context === 'string' && scene !== 'chat_agent' && <MarkdownContext>{formatMarkdownVal(value)}</MarkdownContext>}
          {/* 正在思考 */}
          {thinking && !context && (
          ...
          <div className={cls('opacity-0 h-0 w-0', { 'opacity-100 flex items-center justify-between gap-6 w-auto h-auto': !thinking })}>
            {/* 用户反馈 */}
