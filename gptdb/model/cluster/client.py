...
from gptdb.core.awel import DAGVar
...
from gptdb.model.cluster.manager_base import WorkerManager
...
@register_resource(
    ...
    description=_("Default LLM client(Connect to your GPT-DB model serving)"),
    parameters=[
...
@register_resource(
    ...
    category=ResourceCategory.LLM_CLIENT,
    description=_("Remote LLM client(Connect to the remote GPT-DB model serving)"),
    parameters=[
        Parameter.build_from(
...
class RemoteLLMClient(DefaultLLMClient):
    ...
    If you start GPT-DB model cluster, the controller address is the address of the
    Model Controller(`gptdb start controller`, the default port of model controller
    is 8000).
    Otherwise, if you already have a running GPT-DB server(start it by
    `gptdb start webserver --port ${remote_port}`), you can use the address of the
