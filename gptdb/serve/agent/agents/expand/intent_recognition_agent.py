...
from gptdb.agent.core.profile import DynConfig, ProfileConfig
from gptdb.serve.agent.agents.expand.actions.intent_recognition_action import (
...
class IntentRecognitionAgent(ConversableAgent):
    profile: ProfileConfig = ProfileConfig(
        name=DynConfig(
            "Kevin",
            category="agent",
            key="gptdb_ant_agent_agents_intent_recognition_agent_profile_name",
        ),
        role=DynConfig(
            "Intent Recognition Expert",
            category="agent",
            key="gptdb_ant_agent_agents_intent_recognition_agent_profile_role",
        ),
        goal=DynConfig(
            GOAL_EN,
            category="agent",
            key="gptdb_ant_agent_agents_intent_recognition_agent_profile_goal",
        ),
        retry_goal=DynConfig(
            RETRY_GOAL_EN,
            category="agent",
            key="gptdb_ant_agent_agents_intent_recognition_agent_profile_retry_goal",
        ),
        constraints=DynConfig(
            CONSTRAINTS_EN,
            category="agent",
            key="gptdb_ant_agent_agents_intent_recognition_agent_profile_retry_constraints",
        ),
        retry_constraints=DynConfig(
            RETRY_CONSTRAINTS_EN,
            category="agent",
            key="gptdb_ant_agent_agents_intent_recognition_agent_profile_constraints",
        ),
        desc=DynConfig(
            "Identify user intent.",
            category="agent",
            key="gptdb_ant_agent_agents_intent_recognition_agent_profile_desc",
        ),
    )
