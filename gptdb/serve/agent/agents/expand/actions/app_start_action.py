...
from gptdb.agent import Action, ActionOutput, AgentResource
from gptdb.serve.agent.agents.expand.actions.intent_recognition_action import (
    IntentRecognitionInput,
)
from gptdb.serve.agent.db.gpts_app import GptsApp, GptsAppDao
from gptdb.serve.agent.team.base import TeamMode
from gptdb.vis.tags.vis_plugin import Vis, VisPlugin
        ...
        try:
            ...
            if not gpts_app:
                return ActionOutput(
                    is_exe_success=False,
                    content=ai_message,
                    view=f"[GPTDB Warning] Intent definition application cannot be found [{param.app_code}]{param.app_name}",
                    have_retry=False,
                )
            if TeamMode.NATIVE_APP.value == gpts_app.team_mode:
                return ActionOutput(
                    is_exe_success=False,
                    content=ai_message,
                    view="[GPTDB Warning] Native application connection startup is not supported for the time being.",
                    have_retry=False,
                )
            else:
            ...
            return ActionOutput(
                is_exe_success=False,
                content=ai_message,
                view=f"[GPTDB Warning] An exception occurred during the answering process of linked application [{param.app_code}]{param.intent},{str(e)}",
                have_retry=False,
            )
