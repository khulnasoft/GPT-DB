...
from gptdb.core import ModelMetadata, ModelRequest, PromptTemplate
...
from gptdb.util.json_utils import compare_json_properties_ex, find_json_objects
...
class Service(BaseService[ServeEntity, ServeRequest, ServerResponse]):
    ...
    def get_type_targets(self, prompt_type: str):
        ...
        if type == PromptType.AGENT:
            ...
            from gptdb.rag.evaluation.answer import LLMEvaluationMetric
    ...
    def load_template(
        ...
        if type == PromptType.AGENT:
            ...
            cfg = Config()
            from gptdb.app.scene import AppScenePromptTemplateAdapter
            ...
            if not target:
                raise ValueError("请选择一个场景用来加载模版")
            try:
                from gptdb.rag.evaluation.answer import (
    ...
    def verify_response(
        ...
        if type == PromptType.AGENT:
            try:
                if not target:
                    raise ValueError("请选择一个Agent用来加载模版")
                from gptdb.agent.core import agent_manage
