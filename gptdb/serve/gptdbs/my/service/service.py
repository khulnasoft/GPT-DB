...
from gptdb.serve.core import BaseService
from gptdb.storage.metadata import BaseDao
from gptdb.util.gptdbs.base import INSTALL_DIR
from gptdb.util.gptdbs.repo import (
    copy_and_install,
...
class Service(BaseService[ServeEntity, ServeRequest, ServerResponse]):
    ...
    def install_gpts(
        ...
        try:
            ...
            inner_copy_and_install(repo, name, Path(gptdb_path))
        except Exception as e:
            logger.exception(f"install_gpts failed!{str(e)}")
            raise ValueError(f"Install gptdbs [{type}:{name}] Failed! {str(e)}", e)

        from gptdb.util.gptdbs.base import get_repo_path
        ...
        gptdbs_entity = self.get(ServeRequest(name=name, type=type))

        if not gptdbs_entity:
            ...
            gptdbs_entity.version = base_package.version

            return self.update(ServeRequest(**gptdbs_entity.to_dict()))
    ...
    def uninstall_gpts(
        ...
        try:
            ...
            logger.warning(f"Uninstall gptdbs [{type}:{name}] Failed! {str(e)}", e)
            raise ValueError(f"Uninstall gptdbs [{type}:{name}] Failed! {str(e)}", e)
