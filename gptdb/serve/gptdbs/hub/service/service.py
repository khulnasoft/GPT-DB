...
from gptdb.agent import PluginStorageType
...
from gptdb.util.gptdbs.repo import _install_default_repos_if_no_repos, list_gptdbs
...
class Service(BaseService[ServeEntity, ServeRequest, ServerResponse]):
    ...
    def refresh_hub_from_git(
        ...
        data: List[Tuple[str, str, str, str]] = list_gptdbs()
        ...
        try:
            for repo, package, name, gpts_path in data:
                try:
                    if not name:
                        logger.info(
                            f"gptdbs error repo:{repo}, package:{package}, name:{name}, gpts_path:{gpts_path}"
                        )
                        continue
                    ...
                    if old_hub_info:
                        ...
                        request.author = self._get_gptdbs_author(base_package.authors)
                        request.email = self._get_gptdbs_email(base_package.authors)
    ...
    def _get_gptdbs_author(self, authors):
        pattern = r"(.+?)<"
        names = []
        for item in authors:
    ...
    def _get_gptdbs_email(self, authors):
        pattern = r"<(.*?)>"
        emails: List[str] = []
        for item in authors:
