...
from gptdb.rag.retriever.base import BaseRetriever, RetrieverStrategy
from gptdb.rag.retriever.rerank import DefaultRanker, Ranker
from gptdb.rag.retriever.rewrite import QueryRewrite
...
class EmbeddingRetriever(BaseRetriever):
    ...
    def __init__(
        ...
        Examples:
            .. code-block:: python
                ...
                from gptdb.rag.embedding.embedding_factory import (
    ...
    async def _aretrieve_with_score(
        ...
        if self._query_rewrite:
            with root_tracer.start_span(
                "gptdb.rag.retriever.embeddings.query_rewrite.similarity_search",
                ...
                context = "\n".join([chunk.content for chunk in chunks])
            with root_tracer.start_span(
                "gptdb.rag.retriever.embeddings.query_rewrite.rewrite",
        ...
        with root_tracer.start_span(
            "gptdb.rag.retriever.embeddings.similarity_search_with_score",
            metadata={"query": query, "score_threshold": score_threshold},
        ):
            candidates_with_score = [
        ...
        with root_tracer.start_span(
            "gptdb.rag.retriever.embeddings.rerank",
            metadata={
                "query": query,
    ...
    async def _similarity_search(
        ...
        """Similar search."""
        with root_tracer.start_span(
            "gptdb.rag.retriever.embeddings.similarity_search",
            parent_span_id,
            metadata={
                "query": query,
            },
    ...
    async def _similarity_search_with_score(
        ...
        with root_tracer.start_span(
            "gptdb.rag.retriever.embeddings._do_similarity_search_with_score",
            parent_span_id,
            metadata={
                "query": query,
                "score_threshold": score_threshold,
