...
from gptdb.app.component_configs import initialize_components
...
if CFG.USE_NEW_WEB_UI:
    static_file_path = os.path.join(ROOT_PATH, "gptdb", "app/static/web")
else:
    static_file_path = os.path.join(ROOT_PATH, "gptdb", "app/static/old_web")
...
def mount_routers(app: FastAPI):
    ...
    from gptdb.app.knowledge.api import router as knowledge_router
    from gptdb.app.llm_manage.api import router as llm_manage_api
    from gptdb.app.openapi.api_v1.api_v1 import router as api_v1
    from gptdb.app.openapi.api_v1.editor.api_editor_v1 import (
        router as api_editor_route_v1,
    )
    from gptdb.app.openapi.api_v1.feedback.api_fb_v1 import router as api_fb_v1
    from gptdb.app.openapi.api_v2 import router as api_v2
    from gptdb.serve.agent.app.controller import router as gpts_v1
    from gptdb.serve.agent.app.endpoints import router as app_v2
    ...
    from gptdb.serve.agent.app.recommend_question.controller import (
...
def _get_webserver_params(args: List[str] = None):
    from gptdb.util.parameter_utils import EnvArgumentParser
...
def initialize_app(param: WebServerParameters = None, args: List[str] = None):
    ...
    setup_logging(
        "gptdb", logging_level=param.log_level, logger_filename=param.log_file
    )

    model_name = param.model_name or CFG.LLM_MODEL
    param.model_name = model_name
    param.port = param.port or CFG.GPTDB_WEBSERVER_PORT
    if not param.port:
        param.port = 5670
...
def run_webserver(param: WebServerParameters = None):
    ...
    with root_tracer.start_span(
        ...
        # TODO
        from gptdb.serve.agent.agents.expand.app_start_assisant_agent import (  # noqa: F401
            StartAppAssistantAgent,
        )
        from gptdb.serve.agent.agents.expand.intent_recognition_agent import (  # noqa: F401
