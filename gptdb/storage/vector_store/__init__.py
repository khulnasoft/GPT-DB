...
def _import_pgvector() -> Tuple[Type, Type]:
    from gptdb.storage.vector_store.pgvector_store import PGVectorConfig, PGVectorStore

    return PGVectorStore, PGVectorConfig


def _import_milvus() -> Tuple[Type, Type]:
    from gptdb.storage.vector_store.milvus_store import MilvusStore, MilvusVectorConfig
...
def _import_weaviate() -> Tuple[Type, Type]:
    from gptdb.storage.vector_store.weaviate_store import (
        WeaviateStore,
...
def _import_oceanbase() -> Tuple[Type, Type]:
    from gptdb.storage.vector_store.oceanbase_store import (
        OceanBaseConfig,
        OceanBaseStore,
    )
...
def _import_elastic() -> Tuple[Type, Type]:
    from gptdb.storage.vector_store.elastic_store import (
...
def _import_openspg() -> Tuple[Type, Type]:
    from gptdb.storage.knowledge_graph.open_spg import OpenSPG, OpenSPGConfig
