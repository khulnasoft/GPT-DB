...
If your database is a shared database, and you run GPT-DB in multiple instances, 
...
and use `gptdb db migration` command to manage migration scripts.
"""
...
def _ddl_init_and_upgrade(
    ...
    if disable_alembic_upgrade:
        ...
        warn_msg = (
            ...
            "please run webserver with --disable_alembic_upgrade(`python gptdb/app/gptdb_server.py "
            "--disable_alembic_upgrade`).\n"
            "we suggest you to use `gptdb db migration` to initialize and upgrade database metadata with alembic, "
            "your can run `gptdb db migration --help` to get more information."
        )
