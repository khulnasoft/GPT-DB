"""AWEL: Simple llm client example
    ...
    Examples:
        ...
        .. code-block:: shell
            ...
            MODEL="gpt-3.5-turbo"
            curl -X POST $GPTDB_SERVER/api/v1/awel/trigger/examples/simple_client/chat/completions \
        ...
        Call model and count token.
         .. code-block:: shell
            ...
            curl -X POST $GPTDB_SERVER/api/v1/awel/trigger/examples/simple_client/count_token \
            -H "Content-Type: application/json" -d '{
                "model": "'"$MODEL"'",
                "messages": "hello"
            }'
...
from gptdb.core.awel import DAG, BranchJoinOperator, HttpTrigger, MapOperator
from gptdb.core.operators import LLMBranchOperator, RequestBuilderOperator
...
with DAG("gptdb_awel_simple_llm_client_generate") as client_generate_dag:
...
with DAG("gptdb_awel_simple_llm_client_count_token") as client_count_token_dag:
    # Receive http request and trigger dag to run.
    trigger = HttpTrigger(
