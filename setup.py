...
def get_cpu_avx_support() -> Tuple[OSType, AVXType]:
    ...
    env_cpu_avx = AVXType.of_type(os.getenv("GPTDB_LLAMA_CPP_AVX"))
...
def core_requires():
    ...
    # Just use by GPT-DB internal, we should find the smallest dependency set for run
    # we core unit test.
...
def observability_requires():
    """
    pip install "gptdb[observability]"

    Send GPT-DB traces to OpenTelemetry compatible backends.
    """
...
setuptools.setup(
    ...
    url="https://github.com/khulnasoft/GPT-DB",
    ...
    entry_points={
        "console_scripts": [
            "gptdb=gptdb.cli.cli_scripts:main",
        ],
    },
)
